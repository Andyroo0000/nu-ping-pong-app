...
- **Tournaments** (`app/tournaments`, migration 0021, `lib/bracket.ts`) —
  ...
  **How a tournament is actually run** (migration 0022). One person types in
  who turned up, makes the bracket, keys in scores as games finish, and sends
  ...
  - **Results are published at the end.** `report_tournament_match` records
    ...
    the rating-bearing pending matches in one go and notifies everyone who
    has to confirm. It's idempotent, so a late correction can be sent with
    another tap.

  The integrity property survives: a published result is *pending*, so no
  rating moves until the other side confirms and an organiser still can't hand
  themselves rating. What changes is attribution — 0017 requires
  `reported_by = player_a`, so an organiser-entered result is filed with the
  **winner** as `player_a` and the loser holding the confirm button. The home
  page names the tournament on those, so nobody wonders why they're being
  asked to confirm a match they don't remember anyone reporting.
